...
export class HowToPlayScene extends Phaser.Scene {
  ...
  create(): void {
    ...
    const bodyStyle: Phaser.Types.GameObjects.Text.TextStyle = {
      fontSize: L.fontSize('small'), fontFamily: 'Arial', color: '#ccccdd',
      wordWrap: { width: L.w * 0.35 },
    };

    // ─── Controls ───────────────────────
    this.add.text(leftX, L.y(0.08), 'CONTROLS', sectionStyle).setOrigin(0.5);

    // Touch controls
    this.add.text(leftX, L.y(0.14), 'Touch / Mobile:', {
      fontSize: L.fontSize('small'), fontFamily: 'Arial', color: '#ffffff',
    }).setOrigin(0.5);
    this.add.text(leftX, L.y(0.19), 'Left side: Move\nRight side: Jump', bodyStyle).setOrigin(0.5);
    ...
    // Keyboard controls
    this.add.text(leftX, L.y(0.34), 'Keyboard:', {
      fontSize: L.fontSize('small'), fontFamily: 'Arial', color: '#ffffff',
    }).setOrigin(0.5);
    this.add.text(leftX, L.y(0.42), 'Arrow keys: Move\nSpace: Jump\nZ: Kick\nX: Super move', bodyStyle).setOrigin(0.5);

    // ─── Super Moves ────────────────────
    this.add.text(rightX, L.y(0.08), 'SUPER MOVES', sectionStyle).setOrigin(0.5);
    this.add.text(rightX, L.y(0.16), 'Fill the super meter by kicking\nthe ball. When full, press the\nSuper button to unleash a\npowerful special move!', bodyStyle).setOrigin(0.5);

    // Draw super meter icon
    const meterW = L.unit(0.12);
    const meterH = L.unit(0.018);
    const meterY = L.y(0.26);
    ...
    // ─── Online ─────────────────────────
    this.add.text(rightX, L.y(0.45), 'ONLINE PLAY', sectionStyle).setOrigin(0.5);
    this.add.text(rightX, L.y(0.53), 'Create a room and share the\n4-letter code with a friend,\nor enter their code to join.\nBoth players ready up to start!', bodyStyle).setOrigin(0.5);
