...
export class HowToPlayScene extends Phaser.Scene {
  ...
  create(): void {
    ...
    this.add.text(L.cx, L.y(0.05), 'HOW TO PLAY', {
    ...
    const sectionStyle = { fontSize: L.fontSize('heading'), fontFamily: 'Arial Black, Arial', color: '#ff5500' };
    const bodyStyle: Phaser.Types.GameObjects.Text.TextStyle = {
      fontSize: L.fontSize('body'), fontFamily: 'Arial', color: '#ccccdd',
    ...
    // Touch controls
    this.add.text(leftX, L.y(0.22), 'Touch / Mobile:', {
      fontSize: L.fontSize('body'), fontFamily: 'Arial', color: '#ffffff',
    }).setOrigin(0.5);
    this.add.text(leftX, L.y(0.28), 'Left side: Move\nRight side: Jump', bodyStyle).setOrigin(0.5);

    // Draw touch areas
    const boxW = L.unit(0.11);
    const boxH = L.unit(0.06);
    const boxGap = L.unit(0.015);
    const boxY = L.y(0.34);
    ...
    // ─── Online ─────────────────────────
    this.add.text(rightX, L.y(0.46), 'ONLINE PLAY', sectionStyle).setOrigin(0.5);
    this.add.text(rightX, L.y(0.56), 'Create a room and share the\n4-letter code with a friend,\nor enter their code to join.\nBoth players ready up to start!', bodyStyle).setOrigin(0.5);

    // ─── Goal ───────────────────────────
    this.add.text(leftX, L.y(0.66), 'OBJECTIVE', sectionStyle).setOrigin(0.5);
    this.add.text(leftX, L.y(0.76), 'Score goals by kicking the ball\ninto the opponent\'s net.\nFirst to 5 goals or highest\nscore when time runs out wins!', bodyStyle).setOrigin(0.5);

    // Back button
    const btn = L.button('small');
    createButton(this, L.cx, L.y(0.93), '\u2190 BACK', () => {
