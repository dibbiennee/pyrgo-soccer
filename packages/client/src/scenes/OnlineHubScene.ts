...
export class OnlineHubScene extends Phaser.Scene {
  ...
  create(): void {
    ...
    this.statusText = this.add.text(L.cx, L.y(0.24), '', {
      fontSize: L.fontSize('body'), fontFamily: 'Arial', color: THEME.secondaryHex,
    }).setOrigin(0.5);
    ...
    createButton(this, leftX, L.y(0.32), 'CREA STANZA', () => this.doCreateRoom(), {
      width: btnNormal.width, height: btnNormal.height, style: 'success',
    });

    // Room code display (shown after creation)
    this.roomCodeDisplay = this.add.text(leftX, L.y(0.42), '', {
      fontSize: L.unit(0.14) + 'px', fontFamily: 'Courier New, monospace', color: THEME.primaryHex,
      stroke: '#000000', strokeThickness: 4,
    ...
    this.copyBtn = createButton(this, leftX, L.y(0.52), 'COPIA CODICE', () => this.copyRoomCode(), {
      width: btnSmall.width, height: btnSmall.height, fontSize: L.fontSize('small'), style: 'primary',
    });
    ...
    this.cancelBtn = createButton(this, leftX, L.y(0.68), 'CANCEL', () => {
      ...
      this.statusText?.setText('').setColor(THEME.secondaryHex);
    }, { width: btnSmall.width, height: btnSmall.height, fontSize: L.fontSize('small'), style: 'danger' });
    this.cancelBtn.container.setVisible(false);
    ...
    this.roomCodeText = this.add.text(rightX, L.y(0.36), '____', {
      fontSize: L.unit(0.09) + 'px', fontFamily: 'Courier New, monospace', color: THEME.primaryHex,
      stroke: '#000000', strokeThickness: 3,
    }).setOrigin(0.5);

    // Virtual keyboard (rounded rects)
    ...
    }, { width: btnSmall.width * 0.7, height: btnSmall.height, fontSize: L.fontSize('small'), style: 'danger' });
    ...
    }, { width: btnSmall.width * 0.8, height: btnSmall.height, fontSize: L.fontSize('small'), style: 'success' });

    // Divider line
    const divider = this.add.graphics();
    divider.lineStyle(1, THEME.cardBorder, 0.5);
  ...
  private setupSocketListeners(): void {
    this.socket.on('ROOM_CREATED', (data: { roomCode: string }) => {
      ...
      this.statusText?.setText('Room created!').setColor(THEME.successHex);
    });
    ...
    this.socket.on('ERROR', (data: { code?: string; message: string }) => {
      this.statusText?.setText(data.message).setColor(THEME.dangerHex);
  ...
  private doCreateRoom(): void {
    ...
    if (!navigator.onLine) {
      this.statusText?.setText('Connection required for multiplayer').setColor(THEME.dangerHex);
      return;
    }
    this.state = 'creating';
    this.statusText?.setText('Creating room...').setColor(THEME.secondaryHex);
    this.socket.connect();
  ...
  private doJoinRoom(code: string): void {
    ...
    this.state = 'joining';
    this.statusText?.setText(`Joining ${code}...`).setColor(THEME.secondaryHex);
    this.socket.connect();
  ...
  private copyRoomCode(): void {
    ...
    if (code && navigator.clipboard) {
      navigator.clipboard.writeText(code).then(() => {
        this.statusText?.setText('Code copied!').setColor(THEME.successHex);
  ...
  private createKeyButton(x: number, y: number, letter: string, keySize: number): void {
    ...
    gfx.fillRoundedRect(x - keySize / 2, y - keySize / 2, keySize, keySize, radius);
    gfx.lineStyle(1, THEME.cardBorder, 1);
    gfx.strokeRoundedRect(x - keySize / 2, y - keySize / 2, keySize, keySize, radius);
